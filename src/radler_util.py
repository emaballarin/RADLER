...
def dictmodel_unflatten(flatmodel: th.Tensor, as_dictmodel: collections.OrderedDict):
    ...
    for key in as_dictmodel:
        ...
        dictmodel.update({key: flatmodel[start:stop].view_as(as_dictmodel.get(key)).to(as_dictmodel.get(key).device)})
        start = stop

    return collections.OrderedDict(dictmodel)


def model_weightload(dictmodel: collections.OrderedDict, instantiated_model, th_device=None, only_eval: bool = True):
